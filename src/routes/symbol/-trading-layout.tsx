...
import { MOCK_BASE_BTC, MOCK_BOTS, MOCK_CHANGE_PCT, MOCK_ORDER_BOOK_STATE, MOCK_PORTFOLIO_SUMMARY, MOCK_TRADING_TRADES } from "@/lib/mock-data";
...
interface TradingLayoutProps { symbol: string }
...
export function TradingLayout({ symbol }: TradingLayoutProps) {
	...
	const handleOrderSubmit = async (data: OrderFormData) => {
		...
		try {
			await new Promise<void>((resolve, reject) => { setTimeout(() => { void reject; resolve(); }, 600); });
	...
	const timeframeTabs = (
		<div className="flex items-center gap-1">
			{["1m", "5m", "15m", "1h", "4h", "1d"].map((tf) => (
				<button key={tf} type="button" onClick={() => setActiveTimeframe(tf)}
		...
		<ErrorBoundary>
			<div className="w-full px-3 pb-3 flex flex-col gap-0">
				...
				<Suspense fallback={<div className="w-full h-[600px] grid grid-cols-12 gap-2 p-3">{[...Array(6)].map((_, i) => <div key={i} className="col-span-4 h-[200px] rounded-md bg-muted animate-pulse" />)}</div>}>
					<TradingGrid className="layout" layouts={layouts} breakpoints={{ lg: 1200, md: 996, sm: 768 }} cols={{ lg: 12, md: 10, sm: 6 }} rowHeight={60} margin={[8, 8]} draggableHandle=".cursor-move" onLayoutChange={handleLayoutChange}>
						<div key="book"><ErrorBoundary><Panel title="Order Book"><OrderBook state={MOCK_ORDER_BOOK_STATE} /></Panel></ErrorBoundary></div>
						<div key="chart"><Panel title="Price Chart" noScroll headerExtra={timeframeTabs}><div className="flex-1 p-2 min-h-0"><CandleChart /></div></Panel></div>
						<div key="order"><ErrorBoundary><Panel title="Place Order"><div className="p-3"><OrderForm symbol={symbol} onSubmit={handleOrderSubmit} isLoading={orderSubmitting} /></div></Panel></ErrorBoundary></div>
						<div key="portfolio">
							<ErrorBoundary>
						...
						<div key="bots"><ErrorBoundary><Panel title="Bots"><BotManagerPanel bots={bots} onStatusChange={(id: string, s: BotStatus) => setBots((prev) => prev.map((b) => b.id === id ? { ...b, status: s } : b))} /></Panel></ErrorBoundary></div>
						<div key="trades"><Panel title="Recent Trades"><RecentTradesTable trades={MOCK_TRADING_TRADES} /></Panel></div>
					</TradingGrid>
				</Suspense>
			</div>
		</ErrorBoundary>
