...
const THEMES: { id: ThemeId; label: string; accent: string }[] = [
  { id: "soft", label: "Soft", accent: "var(--primary)" },
...
function useTheme() {
  ...
  // Read initial values from <html> attrs after mount (safe for SSR/jsdom)
  useEffect(() => {
    ...
    if (dt && THEMES.some((t) => t.id === dt)) setThemeState(dt);
    if (dm === "light" || dm === "vibrant") setModeState(dm);
  }, []);
