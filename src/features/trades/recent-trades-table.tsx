...
<th key={h} className={cn("px-3 py-1.5 font-medium", i >= 2 && "text-right")}>{h}</th>
))}
</tr>
</thead>
<tbody>
{trades.map((t) => (
<tr key={`${t.time}-${t.price}`} className="border-b border-border/40 hover:bg-muted/30 transition-colors">
<td className="px-3 py-1 text-muted-foreground">{t.time}</td>
<td className={cn("px-3 py-1 uppercase font-semibold", t.side === "buy" ? "text-trading-bid" : "text-trading-ask")}>{t.side}</td>
<td className={cn("px-3 py-1 text-right", t.side === "buy" ? "text-trading-bid" : "text-trading-ask")}>
...
<td className="px-3 py-1 text-right text-muted-foreground">{t.total.toLocaleString("en-US", { minimumFractionDigits: 2 })}</td>
<td className={cn("px-3 py-1 text-right font-semibold", t.pnl >= 0 ? "text-trading-profit" : "text-trading-loss")}>
